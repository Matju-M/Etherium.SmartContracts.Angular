...
contract('Stores', function (accounts) {
    ...
    /**
     * Adds only one store, checks the saved values and checks that only a store manager is able to add an item.
     */
    ...
    /**
     * Update a store, checks that it updates and only a store manager can update a store.
     */
    ...
    /**
     * Adds a store item for a store, checks that only a store manager is able to, checks values are correct.
     */
    ...
    /**
    * updates a store item for a store, checks that only a store manager is able to, checks values are correct.
    */
    ...
    /**
     * buys an item, checks that when quantity is 0, availability is set to false;
     */
    it("should buy item and reduce quantity", async () => {
        await instance.updateItem(1, 1, "www.updated.com", "pr2", 1, 1, true, { from: storeManager });
